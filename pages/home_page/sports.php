...
<article>
	...
	<h2 class="big-title">Latest Sports News</h2>
            <?php 
                $sql = "SELECT * FROM articles WHERE category_id = 1";
                $result = $conn->query($sql);
                
                if ($result->num_rows > 0): ?>               
                <?php while ($row = $result->fetch_assoc()): ?>  
                    <h3><?= htmlspecialchars($row['title']) ?></h3>
                    <p><?= htmlspecialchars(substr($row['content'], 0, 1000000)) ?>...</p>
                <?php endwhile; ?>
            <?php else: ?>
                <p>No sports news articles available.</p>
            <?php endif; ?>
        <?php endif; ?>
    </div>
</article>
